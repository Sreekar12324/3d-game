...
function createThrustSound() {
    if (sounds.thrustOscillator) return;
...
const keys = {};
const inputState = { pitch: 0, yaw: 0, thrust: 0 };
...
const planet = new THREE.Mesh(
    ...
    new THREE.MeshStandardMaterial({ color: 0x2244ff, roughness: 0.7, metalness: 0.2, emissive: 0x001133, emissiveIntensity: 0.2 })
);
scene.add(planet);
...
function animate() {
    ...
    if (isBoosting && !keys.wasBoost) playBoostSound();
    ...
    if (keys['Space'] && gameState.fuel > 0) {
        ...
        if (isBoosting) { targetCameraDistance = CAMERA.boostDistance; targetFOV = CAMERA.boostFOV; }
        else { targetCameraDistance = CAMERA.normalDistance; targetFOV = CAMERA.normalFOV; }
    } else {
